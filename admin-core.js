...
function costruisci() {
  ...
  var s6label = el('div'); s6label.appendChild(el('span','opt-lbl','Label percorso “6”')); s6label.appendChild(inp('text','degu-label-6', m.degustazione.percorso_label_6 || '6')); s6wrap.appendChild(s6label);
  ...
  var s7label = el('div'); s7label.appendChild(el('span','opt-lbl','Label percorso “7”')); s7label.appendChild(inp('text','degu-label-7', m.degustazione.percorso_label_7 || '7')); s7wrap.appendChild(s7label);
  ...
  if (m.orario) {
    var fso = el('div','fs');
    fso.appendChild(el('div','fs-head','Foglio Orario'));
    var bdo = el('div','fs-body');
    bdo.appendChild(el('div','sub','Immagine QR code'));
    var qrRow = el('div'); qrRow.style.cssText = 'margin-bottom:.8rem';
    var qrLabel = el('span','opt-lbl','URL immagine QR (lascia vuoto per usare il QR predefinito)');
    qrLabel.style.display = 'block';
    qrLabel.style.marginBottom = '.3rem';
    var qrInp = inp('text','orario-qr-url', m.orario.qr_url || '');
    qrInp.style.width = '100%';
    qrInp.placeholder = 'https://esempio.com/qr.png';
    qrRow.appendChild(qrLabel);
    qrRow.appendChild(qrInp);
    // Anteprima immagine attuale
    if (m.orario.qr_url) {
      var qrPreview = el('img','orario-qr');
      qrPreview.src = m.orario.qr_url;
      qrPreview.style.cssText = 'width:60px;height:60px;display:block;margin-top:.4rem;border:1px solid var(--rule)';
      qrRow.appendChild(qrPreview);
    }
    bdo.appendChild(qrRow);
    fso.appendChild(bdo); wrap.appendChild(fso);
  }
}
...
function leggi() {
  ...
  if (m.degustazione) {
    ...
    var l6 = document.getElementById('degu-label-6');
    if (l6) m.degustazione.percorso_label_6 = l6.value.trim() || '6';
    var l7 = document.getElementById('degu-label-7');
    if (l7) m.degustazione.percorso_label_7 = l7.value.trim() || '7';
  }
  ...
  if (m.orario) {
    ...
    if (qrEl) {
      var qrVal = qrEl.value.trim();
      if (qrVal) m.orario.qr_url = qrVal;
      else delete m.orario.qr_url;
    }
  }
  if (m.allergeni) {
