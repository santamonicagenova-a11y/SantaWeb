...
- **Revisione Oppositiva (3ª passata, pool multi-IA vendor diversi)**: **tentata, non eseguibile in questa sessione** — né le chiavi API (`GEMINI_API_KEY`/`GROQ_API_KEY`/`MISTRAL_API_KEY`) né lo script `scripts/p3_pool.py` della skill risultavano disponibili nell'ambiente. Proposta all'utente la modalità manuale (BRIEF PER REVISORE da incollare a mano in una IA esterna) come alternativa; **l'utente ha scelto esplicitamente di saltarla per ora** (non un'omissione: skip approvato). **Debito dichiarato e approvato dall'utente** — resta da fare prima di considerare il modulo definitivo, in particolare su: race condition sul progressivo mensile in caso di doppio inserimento simultaneo (mitigata da un indice univoco DB ma non testata sotto concorrenza), e sulla logica di rollback applicativo in `tracciabilita_carico_create`.
