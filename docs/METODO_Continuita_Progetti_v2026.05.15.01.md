...
> **Ogni file prodotto per il progetto deve avere la versione aggiornata** in formato `v YYYY.MM.DD.NN`, sia in header sia in footer/UI quando applicabile. **La nuova versione va comunicata esplicitamente all'utente quando si presenta il file.** Questa regola va riportata in ogni futuro documento di continuità/handover generato.

### 6.1 Ambito di applicazione

La regola si applica a **tutti i file**, senza eccezioni:
- **File di deploy** (HTML, JS, CSS, JSON dati, asset configurazione) — versione in header + footer/UI
- **Documenti di continuità** (handover, changelog, roadmap, manuali utente, metodologie) — versione in header + footer + **nel nome del file**
- **Documenti tecnici accessori** (specifiche, brief, schemi, README) — versione in header + footer

### 6.2 Versione anche nel nome file (solo documenti di continuità)
...
Esempi:
- `ROADMAP_<Progetto>_v2026.05.15.02.md`
- `HANDOVER_<Progetto>_v2026.05.16.01.md`
- `CHANGELOG_<Progetto>_v2026.05.20.01.md`
- `MANUALE_UTENTE_<Progetto>_v2026.06.15.01.md`
- `METODO_<Nome>_v2026.05.15.01.md`

I **file di deploy** (HTML/JS/CSS/JSON renderizzati nel sito o caricati nel repo) **NON** portano la versione nel nome (rompe URL e riferimenti cross-page). La versione resta solo in header + footer/UI.

### 6.3 Comportamento ad ogni release di documenti di continuità

**Sostituzione**: il file con versione precedente viene rimosso dalla Knowledge Base, sostituito dal nuovo con versione aggiornata. Lo storico è preservato tramite Git. La KB mantiene solo la versione vigente di ogni documento.

Questo evita:
- Confusione su quale versione sia attuale
- Lista KB che cresce indefinitamente
- Riferimenti incrociati ambigui

### 6.4 Propagazione regola
...
Tipicamente la regola viene inclusa:
- Nella sezione "Regola operativa fissa" della ROADMAP corrente
- Nella sezione "Contesto essenziale" di ogni HANDOVER
- Come capitolo dedicato del METODO_Continuita_Progetti

### 6.5 Formato versione
...
Esempi: `v 2026.05.13.01`, `v 2026.05.13.02`, `v 2026.05.14.01`, `v 2026.05.15.02`.

### 6.6 Storia della regola

| Versione regola | Data | Modifica |
|---|---|---|
...
**Inizio sessione:**
- [ ] CHANGELOG (versione vigente) e ultimo HANDOVER caricati in KB Claude
- [ ] Prompt di setup incollato

**Fine sessione:**
- [ ] HANDOVER nuovo prodotto e validato
- [ ] HANDOVER nominato `HANDOVER_<Progetto>_vYYYY.MM.DD.NN.md` con versione nel nome
- [ ] Header HANDOVER contiene `**Versione:** v YYYY.MM.DD.NN` · footer contiene versione
- [ ] CHANGELOG aggiornato (tabella STATO CORRENTE + nuova voce in cima a STORICO) · versione bumpata
- [ ] Versione CHANGELOG nel nome aggiornata (es. `CHANGELOG_<Progetto>_v2026.MM.DD.NN.md`)
- [ ] File deploy modificati hanno versione aggiornata in header + footer/UI (NON nel nome)
- [ ] Commit Git con messaggio `docs: handover YYYY-MM-DD`
- [ ] File nuovi caricati nella KB Claude · file vecchi (versione precedente di stesso documento) **rimossi** dalla KB
- [ ] Versione dichiarata all'utente nell'output di ogni file consegnato
